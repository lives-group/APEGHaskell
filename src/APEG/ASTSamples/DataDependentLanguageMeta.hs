...
module APEG.ASTSamples.DataDependentLanguageMeta where
...
lowLetterMeta :: MAPeg
lowLetterMeta = foldl1 (\e1 e2 -> ( aX e1 |:| aX e2)) (map (mkLit.(:[])) "abcdefghijklmnopqrstuvwxyz")
...
ruleBuildSeq :: ApegRule
ruleBuildSeq = rule "buildSeq"
                        ["g" .:: tLang, "n" .:: tInt]
                        [v "e" .:: tMAPeg]
                          ( "e" .<.  aX lowLetterMeta .:.
